...
function sha256(str) {
  const s = new TextEncoder().encode(str), len = s.length * 8;
  const blk = Math.ceil((len + 1 + 64) / 512) * 64;
  const a = new Uint8Array(blk);
  let i;
  for (i = 0; i < s.length; i++) a[i] = s[i];
  a[i] = 0x80;
  (new DataView(a.buffer)).setUint32(blk - 8, Math.floor(len / 0x100000000));
  (new DataView(a.buffer)).setUint32(blk - 4, len);
  ...
  for (let b = 0; b < blk; b += 64) {
    for (i = 0; i < 16; i++) w[i] = (new DataView(a.buffer)).getUint32(b + i * 4);
    for (i = 16; i < 64; i++) {
...
async function handleLogin() {
  ...
  const password = sha256($('#login-password').value);
...
async function openUserModal(user) {
  ...
  $('#modal-save').addEventListener('click', async () => {
    ...
    if (!editMode) {
      ...
      if (!hasError) data.password = sha256(rawPassword);
    }
